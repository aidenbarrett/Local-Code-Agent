...
def test_positional_evidence_does_not_shift_into_verification_flag():
    result = TaskResult("t1", ProductOutcome.PASS, "Done.", True, ("a:0", "b:1"))
...
def test_failed_check_is_distinct_from_no_check():
    ran = TaskResult("t", ProductOutcome.FAIL, "failed", False, verification_ran=True)
    never = TaskResult("t", ProductOutcome.FAIL, "stopped", False, verification_ran=False)
...
def test_verified_success_requires_verification_to_have_run():
    with pytest.raises(ValueError, match="requires verification_ran"):
        TaskResult("t", ProductOutcome.PASS, "claimed", True, verification_ran=False)


def test_success_still_requires_verified_completion():
    with pytest.raises(ValueError, match="must agree"):
        TaskResult("t", ProductOutcome.PASS, "unproved", False, verification_ran=True)


def test_verification_ran_is_inferred_when_unset():
    assert TaskResult("t", ProductOutcome.PASS, "ok", True).verification_ran is True
    assert TaskResult("t", ProductOutcome.FAIL, "bad", False).verification_ran is False
