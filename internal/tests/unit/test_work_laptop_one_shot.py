...
def test_one_shot_keeps_runtime_pythonhome_out_of_controller_venv():
...
def test_controller_forwards_saved_runtime_python_environment_to_children():
    text = (INTERNAL / "serving" / "serve.py").read_text(encoding="utf-8")
...
def test_one_shot_uses_product_owned_serving_and_qualification():
    text = (INTERNAL / "work-laptop-one-shot.ps1").read_text(encoding="utf-8")
    assert '$Controller = Join-Path $PSScriptRoot "serving\\serve.py"' in text
    assert '$QualificationScript = Join-Path $PSScriptRoot "serving\\qualify_server.py"' in text
...
def test_workstation_summary_explains_what_passed():
    text = (INTERNAL / "work-laptop-one-shot.ps1").read_text(encoding="utf-8")
...
def test_one_shot_reconciles_only_a_verified_controller_owned_live_profile():
    ...
    retry = text.index('& $VenvPython $Controller start @serveArgs --wait-seconds 900', first_start + 1)
