...
RULES: tuple[tuple[str, tuple[str, ...] | None, str, str], ...] = (
    ...
    ("internal/ui/", None, NOT_INSTRUMENT, "future-client documentation"),
    ("internal/vscode/", None, NOT_INSTRUMENT, "future-client documentation"),
...
DECLARED_AREAS = frozenset({
    ...
    "measurement", "personas", "scripts", "skills", "tests", "ui", "vscode",
})
...
def active_files() -> list[Path]:
    """Files under internal/, with provenance's own skip set applied.

    Deliberately every file type, not only .py. The schema that escaped the hashed
    surface until #58 was JSON.
    """
    found = []
...
def test_top_level_areas_match_the_declaration():
    present = {
        ...
        if child.is_dir() and child.name not in provenance._SKIP
    }
