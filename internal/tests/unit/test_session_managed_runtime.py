...
from measurement import managed_runtime
...
def test_session_reuses_matching_owned_runtime(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        managed_runtime.serve,
        "status",
        lambda p: {"healthy": True, "process_alive": True},
    )
    monkeypatch.setattr(managed_runtime.serve, "stop", lambda p: calls.append("stop"))
    monkeypatch.setattr(
        managed_runtime.serve,
        "start",
        lambda *a, **k: calls.append("start") or {"healthy": True},
    )
...
def test_session_stops_only_stale_owned_runtime_before_start(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        managed_runtime.serve,
        "start",
        lambda p, c, wait_seconds: calls.append(("start", c.device, wait_seconds))
        or {"healthy": True},
    )
    ...
    assert result.ok is True
    assert calls == ["stop", ("start", "NPU", 900)]
...
def test_session_refuses_unowned_reachable_endpoint(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        managed_runtime.serve,
        "start",
        lambda *a, **k: starts.append(True) or {"healthy": True},
    )
