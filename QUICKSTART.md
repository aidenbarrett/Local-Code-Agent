...
All commands below run from the repository root. Windows commands are shown in PowerShell.
...
This reports what is already installed and what is missing without changing anything. If setup is required:
...
This shows the approved tools available to the model, the installed task procedures / skills, and the things the controller deliberately does not permit.

The supported list comes from the running implementation rather than a hand-written feature list, so it cannot quietly drift from the code.
...
List the available user-facing model choices:
...
The three Qwen3-8B commands use the same model artifact and client while changing only the requested execution device. `qwen3-coder-30b` is a different model on a different profile.

Chat is deliberately separate from Local Code Agent: it gives you a normal model conversation without repository tools, skills or verification.
...
The same model can be sent to the GPU or CPU:
...
The script requests the device explicitly and reports the device OpenVINO actually resolved before running repeated model inference.
...
The timing numbers are observations on an uncontrolled machine, not benchmark results. Controlled performance and energy measurement is separate work.
...
This uses a disposable C++ project and does not involve a model. It shows why Local Code Agent does not treat passing test output as proof by itself:
...
5. Local Code Agent independently rejects that result as stale and identifies the changed source file.
6. Rebuild honestly. Compilation fails, proving the earlier passing tests were not valid evidence for the current source.
...
| `docs/project-history.md` | Preserved research-first README and experimental history |
...
**The accelerator demo refuses immediately.** Check workstation setup with:
...
**A source-identity test fails.** Do not casually regenerate experiment identities. `INSTRUMENT.json` defines the measured behavioural source surface and historical experiment generations must remain reproducible.
